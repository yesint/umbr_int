...
def cyl_n_mean_std(data):
    x = data[data!=0]
    w = 1.0/x
    mean = np.average(x, weights=w)
    var = np.average((x-mean)**2, weights=w)
    return (len(x), mean, math.sqrt(var))
...
class Window:
    ...
    def get_interval(self,interval,Nintervals):
        if Nintervals>0 and interval>=0:
            ...
            self.num, self.mean, self.std = cyl_n_mean_std(self.data[b:e])
        else:
            self.num, self.mean, self.std = cyl_n_mean_std(self.data)
...
def umb_int_init(param_file_path):
    config = tomllib.load(open(param_file_path, 'rb'))
